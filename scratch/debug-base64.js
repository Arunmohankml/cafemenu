const rawBase64 = ``;

const cleanB64 = rawBase64.replace(/\s+/g, '');
const buffer = Buffer.from(cleanB64, 'base64');

// We got a non-integer tag at binary offset 797 in rsaDer, which starts at byte 26.
// So the absolute offset in buffer is 26 + 797 = 823!
// 823 bytes in base64 is around 823 * 4 / 3 = 1097 characters!
console.log("Abs offset in buffer:", 823);
console.log("Bytes around 823:", buffer.slice(820, 830).toString('hex'));
console.log("Char around 823 in hex:", buffer[823]);

// Let's print the base64 characters around position 1097
console.log("Base64 around 1097:", cleanB64.substring(1080, 1110));
